...
class Page{
    ...
    public $rollPage_comment   = 9;// 评论分页栏每页显示的页数
...
//        $this->nowPage    = empty($_REQUEST[$this->p]) ? 1 : intval($_REQUEST[$this->p]);
     ...
     * @return string
     */
    public function show($type='',$first='') {
//        print_r($this->parameter);
        if(0 == $this->totalRows) return '';

        /* 生成URL */
        $this->parameter[$this->p] = '[PAGE]';
        if($type == 'article_second_list'){
//            print_r($this->parameter);exit;
            $this->url = '/'.$first.'/list_'.$this->parameter['id'].'_'.urlencode('[PAGE]').'.html';
//            $this->url = '/news/'.$this->parameter['one'].'/'.$this->parameter['two'].'/'.$this->parameter['three'].'/index/'.urlencode('[PAGE]').'.html';
        }elseif($type == 'tag_list'){

            $this->url = '/tag/list_'.$this->parameter['id'].'_'.urlencode('[PAGE]').'.html';
        }else{
            $this->url = Url::build(ACTION_NAME, $this->parameter);
        }
...
//                    $link_page .= '<span class="current">' . $page . '</span>';
     ...
     * 评论分页
     */
    public function comment_show($type='',$first=''){
        if(0 == $this->totalRows) return '';

        /* 生成URL */
        $this->parameter[$this->p] = '[PAGE]';
        $this->url = '/article/'.$this->parameter['id'].'.html?p='.urlencode('[PAGE]');
        ...
        $up_row  = $this->nowPage - 1;
        if($up_row == 1){
            $up_page = $up_row > 0 ? '<li><a href="' . str_replace('?p=1','',$this->url($up_row)) . '">' . '<' . '</a></li>' : '';
        }else{
            $up_page = $up_row > 0 ? '<li><a href="' . $this->url($up_row) . '">' . '<' . '</a></li>' : '';
        }

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<li><a href="' . $this->url($down_row) . '">' . '>' . '</a></li>' : '';

        //第一页
        $the_first = '';
        if($this->totalPages > $this->rollPage && ($this->nowPage - $now_cool_page) >= 1){
//            $the_first = '<li><a href="' . $this->url(1) . '">' . '<<' . '</a></li>';
            $the_first = '<li><a href="' . str_replace('?p=1','',$this->url(1)) . '">' . '<<' . '</a></li>';
        }

        //最后一页
        $the_end = '';
        if($this->totalPages > $this->rollPage && ($this->nowPage + $now_cool_page) < $this->totalPages){
            $the_end = '<li><a href="' . $this->url($this->totalPages) . '">' . '>>' . '</a></li>';
        }

        //数字连接
        $link_page = "";
        for($i = 1; $i <= $this->rollPage_comment; $i++){
            if(($this->nowPage - $now_cool_page) <= 0 ){
                $page = $i;
            }elseif(($this->nowPage + $now_cool_page - 1) >= $this->totalPages){
                $page = $this->totalPages - $this->rollPage + $i;
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page <= $this->totalPages){
                    if($page == 1){
                        $link_page .= '<li><a href="' . str_replace('?p=1','',$this->url($page)) . '">' . $page . '</a></li>';
                    }else{
                ...
                if($page > 0 && $this->totalPages != 1){
//                    $link_page .= '<span class="current">' . $page . '</span>';
                    $link_page .= '<li class="focus">' . $page . '</li>';
        ...
        $page_str = str_replace(
            ...
            array($this->config['header'], $this->nowPage, $up_page, $down_page, $the_first, $link_page, $the_end, $this->totalRows, $this->totalPages),
            $this->config['theme']);
        return "<div id='Paging' class='ui-paging-container'><ul>{$page_str}</ul></div>";
    }
...
//        print_r($this->parameter);
        ...
        /* 生成URL */
        $this->parameter[$this->p] = '[PAGE]';
        $this->url = '/news/tag_list/'.$this->parameter['id'].'/'.urlencode('[PAGE]').'.html';
